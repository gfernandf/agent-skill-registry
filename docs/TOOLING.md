...
Answers semantic questions over the generated catalog, exploiting cognitive
hints, safety blocks, and properties already present in
`catalog/capabilities.json`.

It is designed for interactive use and is **not** part of the CI pipeline
(read-only, produces no artifacts).

## Commands

| Command | Example | What it answers |
|---------|---------|-----------------|
...
Lists matching capability IDs. When no match is found, suggests available
values.

### compatible

Shows all types the capability produces, then for each type lists the
capabilities that consume it (excluding the source itself).

### coverage

Reports:

- Overall annotation ratio (annotated vs total capabilities).
- Per-type coverage: how many capabilities produce and consume each cognitive
  type defined in `vocabulary/cognitive_types.yaml`.
- Per-role coverage: how many capabilities belong to each role.
- Uncovered types and roles are flagged.

### safety

Reports:

- Safety-block annotation ratio vs side-effecting capabilities.
- Capabilities with `side_effects: true` but no safety block (warnings).
- Per-capability safety summary: trust level, confirmation requirement,
  reversibility, allowed targets, pre/post gates.

### chain

Finds capability paths between two cognitive types:

- **1-hop**: A single capability that consumes the source type and produces
  the target type.
- **2-hop**: Two capabilities where the first consumes the source type and
  produces an intermediate type, and the second consumes that intermediate
  type and produces the target type.

## Design Notes

- The tool is **read-only**: it does not modify any file.
- Loads `catalog/capabilities.json` (generated by `generate_catalog.py`).
- Loads `vocabulary/cognitive_types.yaml` only for the `coverage` command.
- No external dependencies beyond `pyyaml` (already required by other tools).
